...
#define nx 10        // Numero de pontos em x
#define ny 10        // Número de pontos em y
#define nt 10        // Número de passos de tempo
#define Lx 1.0       // dimensao do dominio em x
#define Ly 1.0       // dimensao do dominio em y
#define Tmax 0.1     // Tempo máximo

void printMatriz(char* msg, double mat[nx][ny], int m, int n){
  puts(msg);
  for(int i=0; i<m; i++){
    for(int j=0; j<n; j++){
      printf("%g\t", mat[i][j]);
...
// Variaveis.
    double u[nx][ny];
    double x[nx], y[ny], t[nt];  //declaração para arrays
